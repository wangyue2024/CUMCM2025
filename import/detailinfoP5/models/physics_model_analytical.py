...
class PhysicsModelAnalytical:
    ...
    def evaluate_single_uav_multi_target(
        self, 
        uav_id: str, 
        x_phys: np.ndarray, 
        ...
        """
        评估单架无人机在特定策略下对多个目标的拦截效果。
        这是问题五下层求解器的核心引擎。

        Args:
            uav_id (str): 无人机的ID, e.g., 'FY1'.
            x_phys (np.ndarray): 解码后的物理决策向量。
                格式: [v, theta, t_launch_0, dt_det_0, t_launch_1, dt_det_1, ...]
            task_list (List[Tuple[int, str]]): 任务列表。
                格式: [(smoke_index, missile_id), ...], e.g., [(0, 'M1'), (2, 'M2')]
        ...
        # --- 2. 逐个计算每个已分配弹药的指标 ---
        # 使用一个字典来收集每个目标导弹对应的遮蔽区间
        intervals_per_missile = {'M1': [], 'M2': [], 'M3': []}
        smoke_details_list = []
        total_proxy_cost = 0.0

        for smoke_index, missile_id in task_list:
            # 提取该弹药的投放和引爆时间
            t_launch = x_phys[2 + smoke_index * 2]
            dt_det = x_phys[2 + smoke_index * 2 + 1]
            ...
            # ！！！关键：为这个“弹-目标”对，切换到对应的导弹模型！！！
            # 我们需要一个临时的模型实例或修改现有方法以接受missile_id
            # 为简化，我们假设有一个方法可以做到这一点
            metrics = self._calculate_metrics_for_one_smoke_one_missile(p_det, t_det, missile_id)
            
            individual_time = sum(end - start for start, end in metrics["intervals"])
            
            # 收集信息
            intervals_per_missile[missile_id].extend(metrics["intervals"])
            
            proxy_cost = metrics["proxy_cost"]
            if individual_time <= 0: # 只有未产生遮蔽时才累加代理成本
        ...
        for missile_id, intervals in intervals_per_missile.items():
            if not intervals:
                merged = []
            else:
            ...
            objective_contributions[missile_id] = sum(end - start for start, end in merged)

        # --- 4. 计算用于CMA-ES的标量成本 ---
        # 简单求和作为标量成本的主要部分
        total_weighted_time = sum(objective_contributions.values())
        scalar_cost = -total_weighted_time + self.config.REWARD_SHAPING_PENALTY_FACTOR * total_proxy_cost
    ...
    def _calculate_metrics_for_one_smoke_one_missile(self, p_det: np.ndarray, t_det: float, missile_id: str) -> Dict:
        """
        修改版的内部函数，可以针对特定的导弹计算指标。
        这需要将原函数中写死的 self.p_missile_0, self.v_vec_missile 等替换为传入参数或查找。
        """
        # 获取特定导弹的运动学参数
        p_missile_0 = self.config.MISSILE_INITIAL_POS[missile_id]
        direction_vec_m = self.config.P_FALSE_TARGET - p_missile_0
        u_missile = direction_vec_m / np.linalg.norm(direction_vec_m)
        v_vec_missile = self.config.V_MISSILE * u_missile
        
        # ！！！复用原有的多项式系数计算逻辑，但使用特定导弹的参数！！！
        # 为避免代码重复，理想情况下 _get_all_polynomial_coefficients 也应接受导弹参数
        # 这里为了演示，我们假设它内部可以动态切换
        # 实际上，更好的做法是:
        # coeffs = self._get_all_polynomial_coefficients(p_det, t_det, p_missile_0, v_vec_missile, self.p_target)
        
        # --- 临时的实现 ---
        # 先保存旧的导弹参数
        original_p_missile_0 = self.p_missile_0
        original_v_vec_missile = self.v_vec_missile
        # 临时切换到新导弹
        self.p_missile_0 = p_missile_0
        self.v_vec_missile = v_vec_missile
        # 调用原函数
        metrics = self._calculate_metrics_for_one_smoke(p_det, t_det)
        # 恢复旧的导弹参数，确保线程安全（虽然在多进程中这不是问题）
        self.p_missile_0 = original_p_missile_0
        self.v_vec_missile = original_v_vec_missile
        
        return metrics

    # ！！！为下层CMA-ES优化器暴露的接口！！！
    def cost_function_p5_scalar(
        self, 
        x_phys: np.ndarray, 
        uav_id: str, 
        task_list: List[Tuple[int, str]]
        ...
        """
        为问题五的下层CMA-ES优化器提供的标量成本函数接口。
        【修正版：移除所有硬约束检查，只包含核心成本计算】
        """
        # 这里的x_phys已经是通过wrapper解码后的、满足隐式约束的向量
        # 因此不再需要任何if/else的约束检查
        
        details = self.evaluate_single_uav_multi_target(uav_id, x_phys, task_list)
        return details["scalar_cost"]